  ...
  return <MobileLayout headerLeft={headerLeft} headerRight={headerRight}>
      ...
      <div className="flex flex-col items-center justify-between p-4 min-h-[calc(100vh-3.5rem-5rem-2rem-3rem-3rem)]">
        <div className="flex-grow flex flex-col items-center justify-center w-full">
          {isLoading && <div className="flex flex-col items-center justify-center py-8">
              <Loader2 className="h-16 w-16 animate-spin text-primary mb-6" />
              <p className="text-muted-foreground text-lg">Examining...</p>
            </div>}

          {!isLoading && <div className="flex flex-col items-center text-center p-6">
          ...
          {!isLoading && <p className="text-muted-foreground mt-8 text-center text-sm px-6">
               Press START to use the otoscope. (Feature upcoming)
            </p>}
        </div>

        <div className="w-full flex justify-center pt-6 mt-auto">
          <Button onClick={handleStartTest} disabled={isLoading} className="rounded-full w-16 h-16 bg-primary hover:bg-primary/90 text-primary-foreground flex flex-col items-center justify-center shadow-xl focus:ring-4 focus:ring-primary/50 text-xs p-1" aria-label="Start Otoscope Test">
            {isLoading ? <Loader2 className="h-6 w-6 animate-spin" /> : <span className="font-bold tracking-wider">START</span>}
          </Button>
        </div>
      </div>
    </MobileLayout>;
